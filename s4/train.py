...
def create_train_state(
    ...
    return NormTrain.create(
        apply_fn=model.apply, params=params, tx=tx, batch_stats=params["batch_stats"]
    )
    ...
    def loss_fn(model_params):
        if classification:
            logits, mod_vars = model.apply(
                {"params": model_params["params"], "batch_stats": state.batch_stats},
                batch_inputs,
                rngs={"dropout": rng},
...
def example_train(
    ...
    for epoch in range(epochs):
        ...
        test_loss, test_acc = validate(
            state, model_cls, testloader, classification=classification
        )
